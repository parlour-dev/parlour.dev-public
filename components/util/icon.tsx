...
const iconOptions = {
  ...
  mail: { bi: HiMail, hi: HiMail },
  office: { bi: HiOfficeBuilding, hi: HiOfficeBuilding },
  tina: { bi: TinaIconSvg, hi: TinaIconSvg },
  parlour: { bi: ParlourLogo, hi: ParlourLogo },
};
...
export const Icon = ({
  ...
  const IconSVG =
    iconOptions[iconName][data?.icon?.style === "bi" ? "bi" : "hi"];
  ...
  if (data.style == "circle") {
    return (
      <div
        ...
        className={`relative z-10 inline-flex items-center justify-center flex-shrink-0 ${iconSizeClasses} rounded-full ${
          iconColorClass[data?.color || "green"].circle
        } ${className}`}
      >
    ...
    const iconColorClasses =
      iconColorClass[data?.color || "green"]?.regular ||
